<div class="card card-custom bg-transparent">
    ...
    <div class="card-header py-3 border-0">
        <div class="card-title align-items-start flex-column">
            <h3 class="card-label font-weight-bolder text-dark">{{ __("user.portofolio") }}</h3>
        ...
        <div class="row">
            <div class="col-12 text-center">
                @if ($data['portofolio']->count() == 0)
                    <button type="button" class="btn btn btn-link text-primary text-hover-dark" onclick="modalShow(this)" data-name="portofolio">
                        <i class="ki-outline ki-plus-circle fs-2 text-primary"></i>
                        Tambahkan {{ __("user.portofolio") }}
                    </button>
                @else
                    @foreach ($data['portofolio'] as $item)
                        @php
                            $_file = $data['documents']->where("class_id", $item->id)->first();
                        @endphp
                        <div class="bg-light-secondary border card card-custom mb-5">
                            <div class="card-body">
                                <div class="d-flex justify-content-end position-absolute ms-n5 ms-md-0 text-right" style="width: 95%">
                                    <button type="button" class="btn btn-circle btn-hover-bg-light btn-icon" onclick="modalShow(this)" data-name="portofolio" data-id="{{ $item->id }}" data-act="edit">
                                ...
                                <div class="d-flex flex-column align-items-start">
                                    ...
                                    <span class="font-weight-bolder">Dokumen:</span>
                                    <div>
                                        @if (!empty($_file))
