...
class KjkCrmList extends Controller
{
    ...
    function index(Request $request){
        ...
        if(!empty($request->a)){
            ...
            if($request->a == "table-perusahaan"){
                ...
                foreach($company as $item){
                    ...
                    $col = [];
                    $col['name'] = '<span class="text-dark fw-bold cursor-pointer" data-kt-drawer-show="true" data-kt-drawer-target="#kt_drawer_example_basic" onclick="show_detail(\'#drawer-content\', \'company\', '.$item->id.')">'.$item->company_name.'</span>';
                    $col['contact'] = '<div class="d-flex flex-column">';
                    if (!empty($item->phones)){
                        $col['contact'] .= '<span class="fw-bold">'.$item->phones[0]['phone'].'</span>';
                    } else {
                        $col['contact'] .= '<span class="fw-bold">'.$item->pic_number.'</span>';
                    }

                    if (!empty($item->emails)){
                        $col['contact'] .= '<span class="fw-bold">'.$item->emails[0].'</span>';
                    } else {
                        $col['contact'] .= '<span class="fw-bold">'.$item->email.'</span>';
                    }
                    if (empty($item->phones) && empty($item->pic_number) && empty($item->emails) && empty($item->email)){
                        $col['contact'] = $completeData;
                        $completeness++;
                    }
                    $col['contact'] .= "</div>";
                    $col['client'] = '<div class="d-flex flex-column">';
                    if ($opClient->count() > 0 || $cpClient->count() > 0){
                        $col['client'] .= '<div class="d-flex align-items-center">';
                        foreach ($opClient->take(5) as $op){
                            $col['client'] .= '<div class="symbol symbol-circle symbol-30px me-3" data-bs-toggle="tooltip" data-bs-html="true"
                        ...
                        if ($opClient->count() > 5){
                            $col['client'] .= '<span onclick="openDet(\'op\','.$item->id.')" class="cursor-pointer">...</span>';
                        }
                        $col['client'] .= "</div>";
                        $col['client'] .= '<div class="d-flex align-items-center"><div class="d-flex align-items-center">';
                        foreach ($cpClient->take(3) as $op){
                            $col['client'] .= '<label class="text-nowrap fw-bold" data-bs-toggle="tooltip" data-bs-html="true"
                        ...
                        if ($cpClient->count() > 3){
                            $col['client'] .= '<span onclick="openDet(\'cl\','.$item->id.')" class="cursor-pointer">...</span>';
                        }
                    } else {
                        $col['client'] = $completeData;
                        $completeness++;
                    }
                    $col['client'] .= "</div>";
                    $col['type'] = $item->type ?? $completeData;
                    if (empty($item->type)){
                        $completeness++;
                    }
                    $col['created'] = $users[$item->created_by] ?? "-";
                    $col['updated'] = date("d/m/Y", strtotime($item->updated_at ?? $item->created_at));
                    $col['completeness'] = $completeness > 0 ? 0 : 1;
                    $col['action'] = '<button type="button" class="btn btn-icon btn-sm" data-kt-menu-trigger="click" data-kt-menu-placement="bottom-end" data-kt-menu-offset="0px, 0px">
                                ...
                                <div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-800 py-2 menu-state-bg-light-primary fw-semibold w-auto min-w-200 mw-300px" data-kt-menu="true">
                                    ...
                                    if (!empty($item->parent) || $item->childs->count() > 0){
                                        $col['action'] .= '<div class="menu-item px-3">
                ...
                return DataTables::collection($row)
                    ->rawColumns(['name', 'contact', 'client', 'type', 'action'])
                    ->make(true);
            }

            if($request->a == "table-kontak"){
                $row = [];
                foreach($contacts as $item){
                    $completeness = 0;
                    ...
                    $col = [];
                    $col['name'] = '<div class="d-flex flex-column">
                    ...
                    $col['posisi'] = $item->position ?? $completeData;
                    if (empty($item->position)){
                        $completeness++;
                    }

                    $col['phone'] = $item->no_telp ?? $completeData;
                    if (empty($item->no_telp)){
                        $completeness++;
                    }
                    $col['email'] = $item->email ?? $completeData;
                    if (empty($item->email)){
                        $completeness++;
                    }
                ...
                return DataTables::collection($row)
                    ->rawColumns(['name', 'posisi', 'phone', 'email', 'type', 'action'])
                    ->make(true);
            }

            if($request->a == "table-file"){
                $row = [];
                foreach($files as $item){
                    ...
                    $col = [];
                    $col['name'] = '<a href="javascript:;" data-kt-drawer-show="true" data-kt-drawer-target="#kt_drawer_example_basic" onclick="show_detail(\'#drawer-content\', \'file\', '.$item->id .')"><u>'.$item->file_name .'</u></a>';
                    $col['type'] = strtoupper(end($_ex));
                    $col['file'] = '<a href="'.asset($item->file_address) .'" class="btn btn-sm btn-primary">
                            '.end($fname) .'
                        </a>';
                    if (empty($item->file_url)){
                        $col['url'] =  $completeData;
                        $completeness++;
                    } else {
                        $col['url'] = '<a href="'.$item->file_url .'">'.$item->file_url .'</a>';
                    }

                    if(!empty($opFile)){
                        $col['related_to'] = '<div class="symbol symbol-circle symbol-30px me-3" data-bs-toggle="tooltip" data-bs-html="true"
                        ...
                        $col['related_to'] =  $completeData;
                        $completeness++;
                    }

                    $col['updated'] = date("d/m/Y", strtotime($item->created_at));
                    $col['completeness'] = $completeness > 0 ? 0 : 1;
                    $col['action'] = '<button type="button" class="btn btn-icon btn-sm" data-kt-menu-trigger="click" data-kt-menu-placement="bottom-end" data-kt-menu-offset="0px, 0px">
                            <i class="fa fa-ellipsis-vertical text-dark"></i>
                        </button>
                ...
                return DataTables::collection($row)
                    ->rawColumns(['name', 'file', 'url', 'related_to', 'action'])
                    ->make(true);
            }

            if($request->a == "det"){
                $type = $request->type;
                $cl = $company->where("id", $request->id)->first();

                $opClient = collect($opp[$cl->id] ?? []);
                $cpClient = collect($cp[$cl->id] ?? []);

                $data = $type == "op" ? $opClient : $cpClient;
                $vuew = view("_crm.list._client_det", compact("data", 'type', 'cl'))->render();
        ...
        $created_by['company'] = array_unique($company->pluck("created_by")->toArray());
        $created_by['contacts'] = array_unique($contacts->pluck("created_by")->toArray());
        $created_by['files'] = array_unique($files->pluck("created_by")->toArray());

        return view("_crm.list.index", compact('opp','cp','op_file','products', "leads", "users", 'created_by', "files", "leadsPluck", "province", "city", "company", "contacts", "pPluck", "cPluck", "comPluck", "t", "vars"));
    }
