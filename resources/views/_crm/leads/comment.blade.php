        ...
        <div class="d-flex flex-column">
            ...
            <div class="d-flex justify-content-between comment-header">
                <div class="d-flex">
                    ...
                    @if (count($replies) > 0)
                        <button type="button" class="btn me-3" data-type="{{ $item->content_type }}" data-comment="{{ $item->id }}" data-id="{{ $item->content_id }}" onclick="openReplies(this)">
                            <span class="reply-close">{{ count($replies) }} replies</span>
                            <span class="reply-show" style="display: none">Close</span>
                        </button>
                    @endif
                    @if ($item->user_id == Auth::id())
                        <button type="button" class="btn text-danger me-3" data-id="{{ $item->id }}" onclick="deleteComment(this)">
                            Delete
                        </button>
                    @endif
                </div>
