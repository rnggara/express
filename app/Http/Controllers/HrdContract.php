...
class HrdContract extends Controller
{
    ...
    function view($id){
        ...
        if(!empty($ctid) && !empty($ctid->approved_at)){
            return redirect()->route('hrd.contract.landing', ["type" => "not-available", "id" => base64_encode($ctid->company_id)]);
        }
        ...
        for ($i=0; $i < count($content); $i++) {
            ...
            if(isset($end[1])){
                if($end[0] != "position"){
                    $txt = (isset($cont['fld'][$end[0]])) ? $cont['fld'][$end[0]] : "";
                    ...
                    $end[0] = (isset($cont['fld'][$end[0]])) ? $cont['fld'][$end[0]] : "";
        ...
        $_ediv = Division::find($emp->division);
        $emp->position = $emp->emp_position." ".(empty($_ediv)) ? "" : $_ediv->name;
