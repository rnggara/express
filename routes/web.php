...
Route::get("login-view", "Auth\LoginController@login")->name("login.guest");
...
Route::prefix("cron")->group(function(){
    ...
    Route::get("/check-attendance-record", "Api\AttendanceController@assignAtt");
});

Route::prefix("booking")->group(function(){
    Route::middleware("auth")->group(function(){
        Route::get("/", "ExpressBooking@index")->name("booking.index");
        Route::get("/invoice/{type}/{id}", "ExpressBooking@invoice")->name("booking.invoice");
        Route::get("/add/{code}", "ExpressBooking@add")->name("booking.add");
        Route::get("/delete/{id}", "ExpressBooking@delete")->name("booking.delete");
        Route::post("/refund/", "ExpressBooking@refund")->name("booking.refund");
        Route::get("/views/{code}", "ExpressBooking@getviews")->name("booking.views");
        Route::post("/confirm", "ExpressBooking@confirm")->name("booking.confirm");
        Route::post("/confirm-payment", "ExpressBooking@confirm_payment")->name("booking.confirm_payment");

        Route::post("/pickup-request", "ExpressBooking@pickup_request")->name("booking.pickup_request");
        Route::post("/get-promo", "ExpressBooking@get_promo")->name("booking.get_promo");
        Route::post("/confirm-order", "ExpressBooking@confirm_order")->name("booking.confirm_order");
    });

    Route::post("/cari", "ExpressBooking@cari")->name("booking.cari");
});

Route::get("/cek-resi/{nomor?}", "ExpressBooking@cek_resi")->name("cek.resi");

Route::middleware("auth")->group(function(){
    Route::group(["prefix" => "depos"], function(){
        Route::get("/", "ExpresDeposit@index")->name("deposit.index");
        Route::get("/delete/{id}", "ExpresDeposit@delete")->name("deposit.delete");

        Route::post("/deposit", "ExpresDeposit@deposit")->name("deposit.deposit");
        Route::post("/confirm", "ExpresDeposit@confirm")->name("deposit.confirm");
    });

    Route::group(["prefix" => "order"], function(){
        Route::post("/konfirmasi", "HomeController@order_konfirmasi")->name("order.konfirmasi");
        Route::post("/awb", "HomeController@order_awb")->name("order.awb");
        Route::post("/done", "HomeController@order_done")->name("order.done");
    });

    Route::group(["prefix" => "invoice"], function(){
        Route::get("/", "ExpressInvoice@index")->name("invoice.index");
    });
});
...
Route::group(['middleware' => 'checkConfig'], function () {
    ...
    Route::get('/', "LandingPageController@index")->name("lp");
...
Route::get("personel-api", "Auth\LoginController@personel_api");

Route::get("/faq", "CMSController@faq")->name("faq.page");
Route::get("/about-us", "CMSController@about_us")->name("about.page");
Route::get("/review", "CMSController@review")->name("review.page");
