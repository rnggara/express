...
<div class="row col-md-12 mx-auto m-5">
    ...
    <form action="{{route('pref.sign.save')}}" method="post" enctype="multipart/form-data">
        @php
            $qo_name = "";
            $qo_position = "";
            $qo_image = "";
            $qo_id = 1;
            $prefQo = json_decode($preferences->qo_signature, true);
            if(!empty($prefQo)){
                $qo_id = $prefQo['id'] ?? 1;
                $qo_name = $prefQo['name'];
                $qo_position = $prefQo['position'];
                $qo_image = $prefQo['image'];
            }
        @endphp
        <div class="row">
            <div class="col-12">
                <h3>Signature Quotation</h3>
                <table class="table table-bordered">
                    <tr>
                        <th>Name</th>
                        <th>Position</th>
                        ...
                        <td>
                            <select name="name" class="form-control select2" required>
                                ...
                                @foreach ($users as $item)
                                    <option value="{{ $item->id }}" {{ ($qo_id == $item->id) ?"SELECTED" : "" }}>{{ $item->name }}</option>
                                @endforeach
                            </select>
                        </td>
                        <td><input type="text" class="form-control" name="position" value="{{ $qo_position }}" required id=""></td>
                        <td align="center">
                            @if ($qo_image != "")
                                <img src="{{ str_replace("public", "public_html", asset("images/signature/$qo_image")) }}" alt="" class="max-w-200px">
                            @endif
                            <div class="custom-file">
                                <input type="file" name="image" class="custom-file-input" {{ $qo_image != "" ? "" : "required" }} id="">
