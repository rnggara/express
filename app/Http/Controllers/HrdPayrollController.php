...
class HrdPayrollController extends Controller
{
    ...
    function generate_payroll(){
        ...
        $emp_archive = [];
        $emp_archive_new = [];
        $archive = Hrd_salary_archive::where("archive_period", $m."-".$y)->get();
        $emp_proportional = $archive->pluck('proportional', 'emp_id');
        foreach($archive as $la){
            $lasal = base64_decode($la->salary) + $la->allowance_office;
            $la_total_sal = $lasal + $la->ovt_nom + $la->field_nom + $la->wh_nom + $la->odo_nom + $la->odo_dgr + $la->odo_swt + $la->fld_dgr + $la->fld_swt + $la->voucher + $la->allow_bpjs_tk + $la->allow_bpjs_kes + $la->allow_jshk + $la->bonus;
            $la_thp_total = $la_total_sal + $la->thr - $la->lateness - $la->deduction - $la->deduc_jshk - $la->deduc_bpjs_tk - $la->deduc_pph21 - $la->deduc_bpjs_kes;
    ...
    function InsertToArchive($company_id, $request_type, $request_month, $request_years){
        ...
        $last_archive = Hrd_salary_archive::where("archive_period", ($_m)."-$_y")
        ...
        $archive = $archive = Hrd_salary_archive::where('archive_period', $m."-".$y)
                ...
                foreach ($emp_arc as $key => $value) { //loop
                    if(isset($emp_name[$value->emp_id])){
                        ...
                        if (empty($emp_name) || $emp_name[$value->emp_id] == null){
                            ...
                            $row[] = $emp_name[$value->emp_id]."<br>".$emp_pos[$value->emp_id]."<br><label style='font-style: italic;'>'".$bnumber."</label><a href='".route('payroll.slip.print', ['id' => $value->emp_id, 'period' => $y."-".$m])."' target='_blank' class='btn btn-xs btn-icon btn-secondary'><i class='fa fa-print'></i></a>";//
                        }
                        ...
                        if(isset($emp_last_archive[$value->emp_id]) && $last_per >= $date){
                            ...
                            $lastAllowBpjstk = $this->get_icon($la->allow_bpjs_tk, $allow_bpjs_tk, $_per);
                            $classAllowBpjstk = $this->get_text_class($la->allow_bpjs_tk, $allow_bpjs_tk);

                            $lastAllowJshk = $this->get_icon($la->allow_jshk, $allow_jshk, $_per);
                            $classAllowJshk = $this->get_text_class($la->allow_jshk, $allow_jshk);

                            $lastVoucher = $this->get_icon($la->voucher, $value->voucher, $_per);
                            $classVoucher = $this->get_text_class($la->voucher, $value->voucher);

                            $now_total_sal = $sal + $value->allowance_office + $value->ovt_nom + $fld + $wh + $odo + $ododgr + $odoswt + $flddgr + $fldswt + $value->voucher + $value->allow_bpjs_tk + $value->allow_bpjs_kes + $value->allow_jshk;
                            $la_total_sal = $lasal + $la->ovt_nom + $la->field_nom + $la->wh_nom + $la->odo_nom + $la->odo_dgr + $la->odo_swt + $la->fld_dgr + $la->fld_swt + $la->voucher + $la->allow_bpjs_tk + $la->allow_bpjs_kes + $la->allow_jshk;
                            ...
                            $lastDeducJshk = $this->get_icon($la->deduc_jshk, $deduc_jshk, $_per);
                            $classDeducJshk = $this->get_text_class($la->deduc_jshk, $deduc_jshk);
                            ...
                            $now_thp_total = $now_total_sal + $value->bonus + $value->thr - $value->lateness - $value->deduction - $value->deduc_jshk - $value->deduc_bpjs_tk - $value->deduc_pph21 - $value->deduc_bpjs_kes;
                            $la_thp_total = $la_total_sal + $la->bonus + $la->thr - $la->lateness - $la->deduction - $la->deduc_jshk - $la->deduc_bpjs_tk - $la->deduc_pph21 - $la->deduc_bpjs_kes;
                        ...
                        $row[] = $lastSalary."<span class='text-$classSalary'>".number_format($sal + $value->allowance_office,2)."</span>";
                        ...
                        $row[] = $lastAllowBpjstk."<span class='text-$classAllowBpjstk'>".number_format($allow_bpjs_tk,2)."</span>";
                        $row[] = $lastAllowBpjskes."<span class='text-$classAllowBpjskes'>".number_format($allow_bpjs_kes,2)."</span>";
                        $row[] = $lastAllowJshk."<span class='text-$classAllowJshk'>".number_format($allow_jshk,2)."</span>";
                        $row[] = $lastVoucher."<span class='text-$classVoucher'>".number_format($value->voucher,2)."</span>";

                        $foot['sum_salary'] += $sal;
                        // $foot['sum_ovt'] += $ovt_total;
                        $foot['sum_voucher'] += $value->voucher;
                        $total_sal = $sal + $value->allowance_office + $value->ovt_nom + $fld + $wh + $odo + $ododgr + $odoswt + $flddgr + $fldswt + $value->voucher + $value->allow_bpjs_tk + $value->allow_bpjs_kes + $value->allow_jshk;
                        ...
                        $row[] = $lastDeducJshk."<span class='text-$classDeducJshk'>".number_format($deduc_jshk,2)."</span>";
                        ...
                        $thp_total = $total_sal + $thr_total + $bonus_amt - $sunction - $ln_amt - $deduc_jshk - $deduc_bpjs_tk - $value->deduc_pph21 - $deduc_bpjs_kes;
            ...
            if (count($emp) > 0) {
                foreach ($emp as $key => $value) {
                    ...
                    if(isset($emp_last_archive[$value->id])){
                        ...
                        $now_total_sal = $sal + $value->allowance_office + $ovt_total + $fld + $wh + $odo + $ododgr + $odoswt + $flddgr + $fldswt + $value->voucher + $value->allow_bpjs_tk + $value->allow_bpjs_kes + $value->allow_jshk + $bonus_amt;
                        $la_total_sal = $lasal + $la->ovt_nom + $la->field_nom + $la->wh_nom + $la->odo_nom + $la->odo_dgr + $la->odo_swt + $la->fld_dgr + $la->fld_swt + $la->voucher + $la->allow_bpjs_tk + $la->allow_bpjs_kes + $la->allow_jshk + $la->bonus;
                    ...
                    $foot['sum_salary'] += $sal + $value->allowance_office;
                    $foot['sum_ovt'] += $ovt_total;
                    $foot['sum_voucher'] += $value->voucher;
                    ...
                    $thp = $total_sal + $thr_total - $absence_deduct - $ln_amt - $value->deduc_bpjs_tk - $value->deduc_bpjs_kes - $value->deduc_jshk - $value->deduc_pph21;
                    $xthp = $thp - $fld - $wh - $odo - $ododgr - $odoswt - $fldswt - $flddgr;
                    ...
                    if(isset($emp_last_archive[$value->id])){
                        ...
                        $now_thp_total = $now_total_sal + $thr_total - $sunction - $ln_amt - $deduc_jshk - $deduc_bpjs_tk - $value->deduc_pph21 - $deduc_bpjs_kes;
                        $la_thp_total = $la_total_sal + $la->thr - $la->lateness - $la->deduction - $la->deduc_jshk - $la->deduc_bpjs_tk - $la->deduc_pph21 - $la->deduc_bpjs_kes;
    ...
    public function print_btl(Request $request){
        ...
        $p_archive = $request->y."-".$m."-".$period_archive;
        ...
        if (strtotime($now) >= strtotime($p_archive)){
            ...
            if (count($emp) > 0) {
                foreach ($emp as $key => $value) {
                    ...
                    $foot['sum_ovt'] += $ovt_total;
                    $foot['sum_voucher'] += $value->voucher;
                    $total_sal = $sal + $ovt_total + $fld + $wh + $odo + $ododgr + $odoswt + $flddgr + $fldswt + $value->voucher + $value->allow_bpjs_tk + $value->allow_bpjs_kes + $value->allow_jshk + $value->allowance_office;
                    $foot['sum_tot_salary'] += $total_sal;
                ...
                //    $row[] = number_format($thr_total,2); //THR
                    ...
                    $thp = $total_sal - $sunction - $absence_deduct - $ln_amt - $value->deduc_bpjs_tk - $value->deduc_bpjs_kes - $value->deduc_jshk - $value->deduc_pph21 + $thr_total;
                    $xthp = $thp - $fld - $wh - $odo - $ododgr - $odoswt - $fldswt - $flddgr;
    ...
    function print_new_slip($id, $period){
        ...
        $total_sal = base64_decode($emp->n_basic_salary) + base64_decode($emp->n_house_allow) + base64_decode($emp->n_health_allow) + base64_decode($emp->n_transport_allow) + base64_decode($emp->n_meal_allow) + base64_decode($emp->n_position_allow) + $archive->allow_bpjs_tk + $archive->allow_bpjs_kes + $archive->allow_jshk;
    ...
    function share_payroll(Request $request){
        ...
        $data = [
            "success" => true,
            "url" => route("payroll.index", $pyshare->code)
        ];
